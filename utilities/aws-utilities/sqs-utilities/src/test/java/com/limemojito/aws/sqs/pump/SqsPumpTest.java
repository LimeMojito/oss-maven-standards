...
import static com.limemojito.aws.sqs.SqsSender.*;
...
public class SqsPumpTest {
    ...
    private void assertCommonAttributes(SendMessageBatchRequestEntry entry) {
        assertThat(entry.messageAttributes()).containsKey(ATTRIBUTE_ID);

        final MessageAttributeValue contentType = MessageAttributeValue.builder()
                                                                       .dataType("String")
                                                                       .stringValue("application/json")
                                                                       .build();

        assertThat(entry.messageAttributes()).containsEntry(ATTRIBUTE_CONTENT_TYPE, contentType);
        assertThat(entry.messageAttributes()).containsEntry(ATTRIBUTE_SPRING_CONTENT_TYPE, contentType);

        assertNumberAttribute(entry, ATTRIBUTE_TIMESTAMP);

        assertNumberAttribute(entry, ATTRIBUTE_CONTENT_LENGTH);
    }

    private static void assertNumberAttribute(SendMessageBatchRequestEntry entry, String key) {
        assertThat(entry.messageAttributes()).containsKey(key);
