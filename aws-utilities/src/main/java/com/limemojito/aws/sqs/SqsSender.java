...
public class SqsSender {
    ...
    private Map<String, MessageAttributeValue> sqsAttrFrom(int contentLength,
        ...
        checkValue(attributeMap, "id", UUID::randomUUID);
        checkValue(attributeMap, "timestamp", System::currentTimeMillis);
        checkValue(attributeMap, "contentType", () -> JSON_CONTENT);
        checkValue(attributeMap, "Content-Type", () -> JSON_CONTENT);
        checkValue(attributeMap, "Content-Length", () -> contentLength);
        log.debug("Generated attribute map {}", attributeMap);
        return attributeMap;
    }
    ...
    private SendMessageBatchRequestEntry createBatchEntry(String queueUrl,
        ...
        if (isFifo(queueUrl) && attributes != null) {
            attributes.keySet().forEach(key -> {
                checkFifoOption(entry, attributes, key);
            });
        }
